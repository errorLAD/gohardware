...
exports.getHardwareshops = (req, res, next) => {
  Seller.find()
    .populate("account", "isVerified")
    .sort({ createdAt: -1 })
    .then((sellers) => {
      const sellersFinal = sellers.filter((hardwareshop) => {
        return hardwareshop.account.isVerified === true;
      });
      res.status(200).json({
        hardwareshops: sellersFinal,
...
exports.getHardwareshop = (req, res, next) => {
...
exports.getHardwareshopsByAddress = (req, res, next) => {
  ...
  Seller.find()
    ...
    .sort({ createdAt: -1 })
    .then((sellers) => {
      const sellersVerified = sellers.filter((hardwareshop) => {
        return hardwareshop.account.isVerified === true;
      });
